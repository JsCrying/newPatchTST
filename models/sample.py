...
from layers.Samplelayer import PatchTST_backbone
    ...
    def __init__(self, configs, max_seq_len: Optional[int] = 1024, d_k: Optional[int] = None, d_v: Optional[int] = None,
        ...
        if self.decomposition:
            ...
            self.model_trend = PatchTST_backbone(c_in=c_in, context_window=context_window, target_window=target_window,
                                                 ...
                                                 subtract_last=subtract_last, verbose=verbose, frequency=frequency,sampling_rate=sampling_rate,**kwargs)
            ...
            self.model = PatchTST_backbone(seq_len=seq_len, c_in=c_in, context_window=context_window, target_window=target_window,
                                           patch_len=patch_len, stride=stride,
                                           max_seq_len=max_seq_len, n_layers=n_layers, d_model=d_model,
                                           n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm,
                                           attn_dropout=attn_dropout,
