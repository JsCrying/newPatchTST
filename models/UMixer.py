...
import torch.fft
from layers.Embed_UMixer import PatchEmbedding
from layers.RevIN import RevIN


def S_Correction(x, x_pre, configs):
    ...
    if configs.debug:
        print(f'{x_ifft.shape = }')
        print(f'{x_pre_ifft.shape = }')
...
class channelMix_CI_pat(nn.Module):
    ...
    def forward(self, x):
        o = torch.zeros(x.shape, dtype=x.dtype, device=f'cuda:{self.configs.gpu}')
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.Pnum = int((self.pred_len + self.seq_len - configs.patch_len) / configs.stride + 2)
        ...
        self.revin = RevIN(configs.feature_in)
        self.patch_embedding = PatchEmbedding(
            configs.d_model, configs.patch_len, configs.stride, configs.dropout)
        self.head = Flatten_Head(configs.feature_in, configs.d_model * self.Pnum, configs.pred_len,
    ...
    def forecast(self, x_input, x_mark_input):
        ...
        if self.configs.debug:
            print(f'before revin: {x_input.shape = }')
        x_input = self.revin(x_input, 'norm')
        if self.configs.debug:
            print(f'before predict_linear: {x_input.shape = }')
        x_input = self.predict_linear(x_input.permute(0, 2, 1))
        if self.configs.debug:
            print(f'after predict_linear: {x_input.shape = }')
        # x_input = x_input.permute(0, 2, 1)
        # x_ori = x_ori.permute(0, 2, 1)
        x_input, n_vars = self.patch_embedding(x_input)
        if self.configs.debug:
            print(f'after patch_embedding {x_input.shape = }')
            print(f'{self.Pnum = }')
        x_old, _ = self.patch_embedding(x_ori.permute(0, 2, 1))
        if self.configs.debug:
            print(f'after patch_embedding {x_old.shape = }')
        x_all = torch.zeros([x_input.shape[0],x_input.shape[1],x_input.shape[2],self.layer], device=f'cuda:{self.configs.gpu}')
        ...
        x_input = self.comb(x_all.to(self.comb.weight.device)).squeeze(-1)
        if self.configs.debug:
            print(f'after squeeze(-1) {x_input.shape = }')
            print(f'{x_old.shape = }')
        x_input = S_Correction(self.layer_norm(x_old), self.layer_norm(x_input[:, :x_old.shape[1], :]), self.configs) * x_input
        ...
        if self.configs.debug:
            print(f'complete: {x_input.shape = }')
    ...
    def forward(self, x_input, x_mark_input, dec_inp, batch_y_mark, mask=None):
        if self.configs.debug:
            ...
            print(f'{self.configs.feature_in = }')
        out = self.forecast(x_input.permute(0, 2, 1), x_mark_input)
        return out  # [B, L, C]
