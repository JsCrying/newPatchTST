...
class PatchTST_backbone(nn.Module):
    def __init__(self, seq_len: int, c_in: int, context_window: int, target_window: int, patch_len: int, stride: int,
        ...
        self.Fourier = FourierBlock(seq_len=seq_len, pred_len=target_window, modes=64, mode_select_method='random')
    ...
    def forward(self, z):  # z: [bs x nvars x seq_len]
        ...
        # model
        # z = self.backbone(z)  # z:  [bs, nvars, d_model]

        # z = self.head(z)  # z: [bs x nvars x target_window]

        # z = self.linear(z) #:  [bs, nvars, target_window]
